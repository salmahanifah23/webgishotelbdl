...
//        echo "nilai <br> ";
  //      echo $lat[$i];
    ...
    for ($objek=0 ; $objek<count($o_hotel) ; $objek++) { 
        ...
        $querysearch2="SELECT id, st_x(st_centroid(geom)) as lng, st_y(st_centroid(geom)) as lat, st_distance_sphere(ST_GeomFromText('POINT(".$long_hotel." ".$lat_hotel.")',-1), geom) as jarak FROM culinary_place where st_distance_sphere(ST_GeomFromText('POINT(".$long_hotel." ".$lat_hotel.")',-1), geom) <= 1000"; 
        $hasil2=mysqli_query($conn,$querysearch2);
    ...
    for ($objek=0 ; $objek<count($o_hotel) ; $objek++) { 
        ...
        $querysearch="SELECT id, st_x(st_centroid(geom)) as lng, st_y(st_centroid(geom)) as lat, st_distance_sphere(ST_GeomFromText('POINT(".$long_hotel." ".$lat_hotel.")',-1), geom) as jarak FROM tourism where st_distance_sphere(ST_GeomFromText('POINT(".$long_hotel." ".$lat_hotel.")',-1), geom) <= 500"; 
        $hasil=mysqli_query($conn,$querysearch);

        $querysearch2="SELECT id, st_x(st_centroid(geom)) as lng, st_y(st_centroid(geom)) as lat, st_distance_sphere(ST_GeomFromText('POINT(".$long_hotel." ".$lat_hotel.")',-1), geom) as jarak FROM tourism where st_distance_sphere(ST_GeomFromText('POINT(".$long_hotel." ".$lat_hotel.")',-1), geom) <= 1000"; 
        $hasil2=mysqli_query($conn,$querysearch2);
